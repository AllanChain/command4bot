...
class Command:
    def __init__(self, command_func, keywords, groups,
    ...
    def __call__(self, payload, **kargs):
        return self.command_func(payload, **{
...
class BaseCommandRegistry:
    ...
    def get_similar_commands(self, keyword):
        return (self._reg[match] for match in
                get_close_matches(keyword, self._reg.keys())
...
class CommandRegistry(BaseCommandRegistry):
    ...
    def resolve_command_status(self, command):
        if not self.get_status(command.name):
            return False
        return all(self.get_status(group_name)
...
class CommandsManager:
    def __init__(self, setup_reg=None, command_reg=None, fallback_reg=None,
    ...
    def help_with_similar(self, content):
        ...
        return '\n'.join((TEXT_GENERAL_RESPONSE,
                          TEXT_POSSIBLE_COMMAND, *helps))
